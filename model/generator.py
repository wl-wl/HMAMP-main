...
class Generator(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, output_size,
        ...
        self.decoder = nn.Sequential(nn.Linear(hidden_size, int(hidden_size / 2)), nn.LeakyReLU(0.1),
    ...
    def generate(self, dataloader, prime_str='', end_token='', predict_len=40, temperature=1):  # predict_len=150
        ...
        cell = self.init_cell()
        hidden = (hidden, cell)
        token=['b','a','r','n','d','c','q','e','g','h','i','l','k','m','f','p','s','t','w','y','v','x','z','u','j','o']

        prime_str=random.choice(token)
        end_token=random.choice(token)
    ...
    def train_one_peptides(self, inp, target):
        """
    ...
    def pretrain(self, dataloader, epochs, save_path, eval_loader1=None, eval_loader2=None):
        ...
        for n in trange(1, total_num + 1, desc='Training in progress...', ncols=100):
            ...
            loss = self.train_one_peptides(inp, target)
            epoch_loss += loss
            num += 1
            if num % print_every == 0:
                print('\n-------------------------Epoch loss: [%d/%d (%d%%) %.4f]' % (
                n, total_num, num / total_num * 100, epoch_loss / print_every))
    ...
    def evaluate(self, eval_loader, log_path):
        ...
        nll = NLLLoss()
        total_loss = 0
        for trajectory in eval_loader.peptide_list:
            prob = self.get_prob(eval_loader.char_tensor(trajectory))
            loss = nll(prob).item()
            total_loss += loss

        mean_nll = total_loss / eval_loader.peptide_num
