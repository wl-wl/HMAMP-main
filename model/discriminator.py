...
class Discriminator(nn.Module):
    def __init__(self, input_size, embed_size, hidden_size, use_cuda=None, dropout=0.2,
                 optimizer_instance=torch.optim.Adadelta, lr=0.001):
        """
        Parameters
        ----------
        input_size: int
        ...
        self.hidden2out = nn.Sequential(nn.Linear(2 * 2 * hidden_size, hidden_size), nn.Tanh(), nn.Dropout(p=dropout),
    ...
    def train_epochs(self, data_loader, epochs):
        ...
        minimize the cross-entropy loss when predicting whether the peptides is ground
        truth or generated
        ...
        for epoch in range(0, epochs):
            ...
            for data, length, label in data_loader:
                ...
                self.optimizer.step()
            data_loader.reset()
            print('----------Discriminator epoch: %d/%d  loss:%.4f' % (
            epoch + 1, epochs, epoch_loss / len(data_loader.pairs)))
